...
export const getRecentOffers = async (req: Request, res: Response): Promise<void> => {
  const n = req.body.numberOfOffers;
  Offer.find({}, { __v: 0 }, function (err: any, result: any) {
    if (err) {
      res.send(failResponse('error', {}))
    } else {
  ...
  }).sort({ _id: -1 }).limit(n).populate({ path: 'company', select: 'name -_id' });
}
